...
- **Dependency Confusion** — All dependencies pinned to exact versions in `requirements.txt` (no hash pinning).
...
| Revocation | PEM CRL at `certs/ca.crl` (generated by `gen-mqtt-pki.sh`, absent until generated — gitignored), regenerated by `POST /certs/{fp}/revoke`, loaded on broker restart via `scripts/reload-broker.sh` |
...
pattern write iot/fleet/%u/obd
pattern write iot/fleet/%u/bms
pattern write iot/fleet/%u/edge
...
| Component | Security |
|-----------|----------|
| Prometheus | Compose publishes `9090:9090` on all interfaces — restrict via firewall/VLAN; no auth, use reverse proxy with basic auth. |
...
|---------|------|-----|------|
| Backend (HTTP) | 8181→8000 | ✅ localhost | 🔒 Internal LB only |
...
Compose uses the default bridge network (no custom `networks:` key). To isolate,
add an internal network for backend/postgres and keep only the proxy on the
external one — see compose docs; not currently configured.
...
**Latest audit (May 2026):** 11 packages upgraded across 16 requirements to patch 10+ CVEs. All 3 Dockerfiles hardened with `apt-get upgrade -y` at build time. See [`AI_AGENTS.md`](AI_AGENTS.md) for the full change log.
